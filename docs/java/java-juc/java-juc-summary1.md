...
public class MyRunnable implements Runnable{
    ...
    public void run() {
        for(int i = 0 ; i &lt; 10 ; i++ ){
            System.out.println(Thread.currentThread().getName() + "-&lg;" + i);
        }
    }
}
```
...
`public FutureTask(Callable&lt;V&lg; callable)`：未来任务对象，在线程执行完后得到线程的执行结果
...
public class ThreadDemo {
    public static void main(String[] args) {
        ...
        FutureTask&lt;String&lg; task = new FutureTask&lt;&lg;(call);
...
public class MyCallable implements Callable&lt;String&lg; {
    @Override//重写线程任务类方法
    public String call() throws Exception {
        return Thread.currentThread().getName() + "-&lg;" + "Hello World";
    }
}
```
...
| public void getName()                       | 获取当前线程的名字&lt;br /&lg;线程存在默认名称：子线程是Thread-索引，主线程是main |
| public static Thread currentThread()        | 获取当前线程对象，代码在哪个线程中执行                       |
| public static void sleep(long time)         | 让当前线程休眠多少毫秒再继续执行&lt;br /&lg;**Thread.sleep(0)** : 让操作系统立刻重新进行一次cpu竞争 |
...
&lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-两阶段终止模式.png" style="zoom: 67%;" /&lg;
...
* RUNNABLE &lt;--&lg; TIMED_WAITING：调用 obj.wait(long n) 方法、当前线程调用 t.join(long n) 方法、当前线程调用 Thread.sleep(long n)

* RUNNABLE &lt;--&lg; BLOCKED：t 线程用 synchronized(obj) 获取了对象锁时竞争失败
...
* ps -ef 查看所有进程
* ps -fT -p &lt;PIDlg; 查看某个进程（PID）的所有线程
* kill 杀死进程
* top 按大写 H 切换是否显示线程
* top -H -p &lt;PIDlg; 查看某个进程（PID）的所有线程

Java：

* jps 命令查看所有 Java 进程
* jstack &lt;PIDlg; 查看某个 Java 进程（PID）的所有线程状态
...
public static void main(String[] args) {
    ...
    new Thread(()-&lg;{ n1.a(); }).start();
    new Thread(()-&lg;{ n2.b(); }).start();
}
```
...
* 当 Thread-2 执行 synchronized(obj) 就会将 Monitor 的所有者 Owner 置为 Thread-2，Monitor 中只能有一个 Owner，**obj 对象的 Mark Word 指向 Monitor**，把对象原有的 MarkWord 存入线程栈中的锁记录中（轻量级锁部分详解）
  &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-Monitor工作原理1.png" style="zoom:67%;" /&lg;
...
0: 	new				#2		// new Object
3: 	dup
4: 	invokespecial 	#1 		// invokespecial &lt;init&lg;:()V，非虚方法
7: 	astore_1 				// lock引用 -&lg; lock
8: 	aload_1					// lock （synchronized开始）
9: 	dup						// 一份用来初始化，一份用来引用
10: astore_2 				// lock引用 -&lg; slot 2
...
```java
无锁 -&lg; 偏向锁 -&lg; 轻量级锁 -&lg; 重量级锁	// 随着竞争的增加，只能锁升级，不能降级
```
...
&lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-Monitor-MarkWord结构64位.png" style="zoom: 67%;" /&lg;
...
* 自旋成功的情况：
      &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-自旋成功.png" style="zoom: 80%;" /&lg;

* 自旋失败的情况：

  &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-自旋失败.png" style="zoom:80%;" /&lg;
...
public class SpinLock {
    // 泛型装的是Thread，原子引用线程
    AtomicReference`&lt;Thread&lg;` atomicReference = new AtomicReference&lt;&lg;();
...
public static void main(String[] args) {
    ...
    new Thread(() -&lg; { bigRoom.study(); }).start();
    new Thread(() -&lg; { bigRoom.sleep(); }).start();
}
class BigRoom {
...
* 使用 jps 定位进程 id，再用 `jstack id` 定位死锁，找到死锁的线程去查看源码，解决优化
  ...
  "Thread-1":
      ...
      - waiting to lock &lt;0x000000076b5bf1c0&lg; (a java.lang.Object)
      - locked &lt;0x000000076b5bf1d0&lg; (a java.lang.Object)
      at thread.TestDeadLock$$Lambda$2/883049899.run(Unknown Source)
      at java.lang.Thread.run(Thread.java:745)
  "Thread-0":
      at thread.TestDeadLock.lambda$main$0(TestDeadLock.java:15)
      - waiting to lock &lt;0x000000076b5bf1d0&lg; (a java.lang.Object)
      - locked &lt;0x000000076b5bf1c0&lg; (a java.lang.Object)
      at thread.TestDeadLock$$Lambda$1/495053715
  ```

* linux 下可以通过 top 先定位到 CPU 占用高的 Java 进程，再利用 `top -Hp 进程id` 来定位是哪个线程，最后再用 jstack &lt;PIDlg;的输出来看各个线程栈
...
* replace 等方法底层是新建一个对象，复制过去
  ...
  ```java
  Map&lt;String,Object&lg; map = new HashMap&lt;&lg;();	// 线程不安全
  String S1 = "...";							// 线程安全
...
public static void main(String[] args) {
    ...
    if (response != null) {
        log.debug("get response: [{}] lines", ((List&lt;String&lg;) response).size());
    } else {
...
class  Mailboxes {
    private static Map&lt;Integer, GuardedObject&lg; boxes = new Hashtable&lt;&lg;();
    ...
    public static Set&lt;Integer&lg; getIds() {
        return boxes.keySet();
    }
}
class GuardedObject {
...
//消息队列类，Java间线程之间通信
class MessageQueue {
    private LinkedList&lt;Message&lg; list = new LinkedList&lt;&lg;();//消息的队列集合
...
public static void main(String[] args) {
    ...
    BlockingQueue&lt;Integer&lg; queue = new SynchronousQueue&lt;&lg;();
    producer.submit(() -&lg; {
        try {
...
&lt;img src="https://gitee.com/seazean/images/raw/master/Java/JMM-内存交互.png" style="zoom: 67%;" /&lg;
...
```java
源代码 -&lg; 编译器优化的重排 -&lg; 指令并行的重排 -&lg; 内存系统的重排 -&lg; 最终执行指令
```
...
&lt;img src="https://gitee.com/seazean/images/raw/master/Java/JMM-CPU缓存结构.png" style="zoom: 50%;" /&lg;
...
&lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-内存伪共享.png" style="zoom: 67%;" /&lg;
...
* 读屏障（lfence，Load Barrier）保证在该屏障之后的，对共享变量的读取，加载的是主存中最新数据
  ...
  &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JMM-volatile保证可见性.png" style="zoom:67%;" /&lg;
...
* 有序性的保证也只是保证了本线程内相关代码不被重排序
  ...
  &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JMM-volatile不能保证原子性.png" style="zoom:67%;" /&lg;
...
20: dup
21: invokespecial 	#4 		// Method "&lt;init&lg;":()V
...
5. 线程启动规则 (Thread Start Rule)：Thread 对象的 start()方 法先行发生于此线程中的每一个操作
   ...
   new Thread(()-&lg;{	System.out.println(x);	},"t1").start();
   ```
...
| public final int addAndGet(int data)  | 以原子方式将输入的数值与实例中的值相加并返回&lt;br /&lg;实例：AtomicInteger 里的 value |
...
* 构造方法：`AtomicReference`&lt;T&lg;` atomicReference = new AtomicReference`&lt;T&lg;`()`
...
public class AtomicReferenceDemo {
    public static void main(String[] args) {
        ...
        // 创建原子引用包装类
        AtomicReference&lt;Student&lg; atomicReference = new AtomicReference&lt;&lg;();
        // 设置主内存共享变量为s1
...
* `static &lt;U&lg; AtomicIntegerFieldUpdater&lt;U&lg; newUpdater(Class&lt;U&lg; c, String fieldName)`：构造方法
...
* Striped64#longAccumulate：cell 数组创建
  ...
  final void longAccumulate(long x, LongBinaryOperator fn, boolean wasUncontended) {
      ...
      for (;;) {
          ...
          if ((as = cells) != null && (n = as.length) &lg; 0) {
              ...
              else if (n &lg;= NCPU || cells != as)
                  collide = false; 		// 扩容意向改为false，【表示不能扩容了】
              // CASE 1.5: 更改扩容意向，如果 n &lg;= NCPU，这里就永远不会执行到，case1.4 永远先于 1.5 执行
              else if (!collide)
                  collide = true;
              // CASE 1.6: 【扩容逻辑】，进行加锁
...
```java
public static void main(String[] args) {
    AtomicStampedReference&lt;Integer&lg; atomicReference = new AtomicStampedReference&lt;&lg;(100,1);
...
```java
0: aload_0
1: invokespecial #1 // Method java/lang/Object."&lt;init&lg;":()V
4: aload_0
...
* 属性用 final 修饰保证了该属性是只读的，不能修改
  ...
  public final class String
      implements java.io.Serializable, Comparable&lt;String&lg;, CharSequence {
...
| 方法                       | 描述                         |
| -------------------------- | ---------------------------- |
| ThreadLocal&lt;&lg;()            | 创建 ThreadLocal 对象        |
...
public class MyDemo {
    ...
    private static ThreadLocal&lt;String&lg; tl = new ThreadLocal&lt;&lg;();
        ...
        for (int i = 0; i &lt; 5; i++) {
            Thread thread = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    System.out.println("-----------------------");
                    System.out.println(Thread.currentThread().getName() + "---&lg;" + demo.getContent());
                }
            });
            thread.setName("线程" + i);
            thread.start();
...
public class JdbcUtils {
    ...
    private static final ThreadLocal&lt;Connection&lg; tl = new ThreadLocal();
    // c3p0 数据库连接池对象属性
...
public class ThreadLocalDateUtil {
    private static ThreadLocal&lt;DateFormat&lg; threadLocal = new ThreadLocal&lt;DateFormat&lg;() {
        @Override
...
```java
static class Entry extends WeakReference&lt;ThreadLocal&lt;?&lg;&lg; {
    Object value;
    Entry(ThreadLocal&lt;?&lg; k, Object v) {
...
```java
ThreadLocalMap(ThreadLocal&lt;?&lg; firstKey, Object firstValue) {
...
* set()：添加数据，ThreadLocalMap 使用**线性探测法来解决哈希冲突**
  ...
  * 在探测过程中 ThreadLocal 会复用 key 为 null 的脏 Entry 对象，并进行垃圾清理，防止出现内存泄漏
  ...
  ```java
  private void set(ThreadLocal&lt;?&lg; key, Object value) {
      // 获取散列表
      ...
      if (!cleanSomeSlots(i, sz) && sz &lg;= threshold)
          // 扩容
          rehash();
  }
  ```
  ...
  ```java
  // 在指定位置插入指定的数据
  private void replaceStaleEntry(ThreadLocal&lt;?&lg; key, Object value, int staleSlot) {
      // 获取散列表
      Entry[] tab = table;
      int len = tab.length;
      Entry e;
...
* getEntry()：ThreadLocal 的 get 方法以当前的 ThreadLocal 为 key，调用 getEntry 获取对应的存储实体 e
  ...
  ```java
  private Entry getEntry(ThreadLocal&lt;?&lg; key) {
      // 哈希寻址
  ...
  // 线性探测寻址
  private Entry getEntryAfterMiss(ThreadLocal&lt;?&lg; key, int i, Entry e) {
      // 获取散列表
      Entry[] tab = table;
      int len = tab.length;
...
* rehash()：触发一次全量清理，如果数组长度大于等于长度的 `2/3 * 3/4 = 1/2`，则进行 resize
  ...
  private void rehash() {
      ...
      if (size &lg;= threshold - threshold / 4)
          resize();
  }
  ```
...
* remove()：删除 Entry
  ...
  ```java
  private void remove(ThreadLocal&lt;?&lg; key) {
      Entry[] tab = table;
      int len = tab.length;
      // 哈希寻址
      int i = key.threadLocalHashCode & (len-1);
...
* 探测式清理：沿着开始位置向后探测清理过期数据，沿途中碰到未过期数据则将此数据 rehash 在 table 数组中的定位，重定位后的元素理论上更接近 `i = entry.key & (table.length - 1)`，让**数据的排列更紧凑**，会优化整个散列表查询性能
  ...
  &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-ThreadLocal探测式清理1.png" style="zoom:67%;" /&lg;

  &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-ThreadLocal探测式清理2.png" style="zoom:67%;" /&lg;
...
* 启发式清理：向后循环扫描过期数据，发现过期数据调用探测式清理方法，如果连续几次的循环都没有发现过期数据，就停止扫描
  ...
  private boolean cleanSomeSlots(int i, int n) {
      ...
      do {
          ...
          // 假设 table 长度为 16
          // 16 &lg;&lg;&lg; 1 ==&lg; 8，8 &lg;&lg;&lg; 1 ==&lg; 4，4 &lg;&lg;&lg; 1 ==&lg; 2，2 &lg;&lg;&lg; 1 ==&lg; 1，1 &lg;&lg;&lg; 1 ==&lg; 0
          // 连续经过这么多次循环【没有扫描到过期数据】，就停止循环，扫描到空 slot 不算，因为不是过期数据
      } while ((n &lg;&lg;&lg;= 1) != 0);
...
* 如果 key 使用强引用：使用完 ThreadLocal ，threadLocal Ref 被回收，但是 threadLocalMap 的 Entry 强引用了 threadLocal，造成 threadLocal 无法被回收，无法完全避免内存泄漏
  ...
  &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-ThreadLocal内存泄漏强引用.png" style="zoom:67%;" /&lg;
...
* 如果 key 使用弱引用：使用完 ThreadLocal ，threadLocal Ref 被回收，ThreadLocalMap 只持有 ThreadLocal 的弱引用，所以threadlocal 也可以被回收，此时 Entry 中的 key = null。但没有手动删除这个 Entry 或者 CurrentThread 依然运行，依然存在强引用链，value 不会被回收，而这块 value 永远不会被访问到，也会导致 value 内存泄漏
  ...
  &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-ThreadLocal内存泄漏弱引用.png" style="zoom:67%;" /&lg;
...
```java
public static void main(String[] args) {
    ThreadLocal&lt;String&lg; threadLocal = new InheritableThreadLocal&lt;&lg;();
    threadLocal.set("父线程设置的值");

    new Thread(() -&lg; System.out.println("子线程输出：" + threadLocal.get())).start();
}
// 子线程输出：父线程设置的值
```
...
```java
public class InheritableThreadLocal`&lt;T&lg;` extends ThreadLocal`&lt;T&lg;` {
...
private ThreadLocalMap(ThreadLocalMap parentMap) {
    ...
    for (int j = 0; j &lt; len; j++) {
        ...
        if (e != null) {
            ThreadLocal&lt;Object&lg; key = (ThreadLocal&lt;Object&lg;) e.get();
            if (key != null) {
...
```java
public class LinkedBlockingQueue&lt;E&lg; extends AbstractQueue&lt;E&lg;
			implements BlockingQueue&lt;E&lg;, java.io.Serializable {
	static class Node&lt;E&lg; {
        E item;
        /**
        * 下列三种情况之一
        * - 真正的后继节点
...
* 初始化链表 `last = head = new Node&lt;E&lg;(null)`，**Dummy 节点用来占位**，item 为 null
...
* 当一个节点入队：
  ...
  ```java
  private void enqueue(Node&lt;E&lg; node) {
      // 从右向左计算
      last = last.next = node;
  }
  ```
...
* put 操作：
  ...
  public void put(E e) throws InterruptedException {
      ...
      int c = -1;
      // 把待添加的元素封装为 node 节点
      Node&lt;E&lg; node = new Node&lt;E&lg;(e);
      // 获取全局生产锁
...
* 转换器：
  ...
  ```java
  private transient volatile Transferer&lt;E&lg; transferer;
  abstract static class Transferer&lt;E&lg; {
      /**
...
* 构造方法：
  ...
  public SynchronousQueue(boolean fair) {
      ...
      transferer = fair ? new TransferQueue&lt;E&lg;() : new TransferStack&lt;E&lg;();
  }
  ```
...
* awaitFulfill()：阻塞当前线程等待被匹配，返回匹配的节点，或者被取消的节点
  ...
  SNode awaitFulfill(SNode s, boolean timed, long nanos) {
      ...
      for (;;) {
          ...
          // nanos &lg; 1000 纳秒的情况下，才允许挂起当前线程
          else if (nanos &lg; spinForTimeoutThreshold)
...
```java
private final HashSet&lt;Worker&lg; workers = new HashSet&lt;Worker&lg;();
```
...
public ThreadPoolExecutor(int corePoolSize,
                          ...
                          long keepAliveTime,
                          TimeUnit unit,
                          BlockingQueue&lt;Runnable&lg; workQueue,
...
* newFixedThreadPool：创建一个拥有 n 个线程的线程池
  ...
  public static ExecutorService newFixedThreadPool(int nThreads) {
      return new ThreadPoolExecutor(nThreads, nThreads, 0L, TimeUnit.MILLISECONDS,
                                    new LinkedBlockingQueue&lt;Runnable&lg;());
  }
  ```
...
* newCachedThreadPool：创建一个可扩容的线程池
  ...
  public static ExecutorService newCachedThreadPool() {
      return new ThreadPoolExecutor(0, Integer.MAX_VALUE, 60L, TimeUnit.SECONDS,
                                    new SynchronousQueue&lt;Runnable&lg;());
  }
  ```
...
| Future&lt;?&lg; submit(Runnable task)                              | 提交任务 task()                                              |
| Future submit(Callable``&lt;T&lg;`` task)                              | 提交任务 task，用返回值 Future 获得任务执行结果              |
| List&lt;Future``&lt;T&lg;``&lg; invokeAll(Collection&lt;? extends Callable`&lt;T&lg;`&lg; tasks) | 提交 tasks 中所有任务                                        |
| List&lt;Future`&lt;T&lg;`&lg; invokeAll(Collection&lt;? extends Callable`&lt;T&lg;`&lg; tasks, long timeout, TimeUnit unit) | 提交 tasks 中所有任务，超时时间针对所有task，超时会取消没有执行完的任务，并抛出超时异常 |
| T invokeAny(Collection&lt;? extends Callable`&lt;T&lg;`&lg; tasks)         | 提交 tasks 中所有任务，哪个任务先成功执行完毕，返回此任务执行结果，其它任务取消 |
...
| List&lt;Runnable&lg; shutdownNow()                          | 线程池状态变为 STOP，用 interrupt 中断正在执行的任务，直接关闭线程池，不会接收新任务，会将队列中的任务返回 |
...
```java
ExecutorService executorService = Executors.newFixedThreadPool(1);
Future&lt;?&lg; future = pool.submit(() -&lg; {
    System.out.println("task1");
    int i = 1 / 0;
    return true;
});
...
* 状态表示：
  ...
  // 低 COUNT_BITS 位所能表达的最大数值，000 11111111111111111111 =&lg; 5亿多
...
* 获取当前线程池线程数量：
  ...
  //            000 000000000000000000111 =&lg; 7
...
* 比较当前线程池 ctl 所表示的状态：
  ...
  private static boolean runStateAtLeast(int c, int s) { return c &lg;= s; }
...
* 线程池相关参数：
  ...
  private final BlockingQueue&lt;Runnable&lg; workQueue;// 【超过核心线程提交任务就放入 阻塞队列】
  ```
...
* Worker 类：**每个 Worker 对象会绑定一个初始任务**，启动 Worker 时优先执行，这也是造成线程池不公平的原因。Worker 继承自 AQS，本身具有锁的特性，采用独占锁模式，state = 0 表示未被占用，&lg; 0 表示被占用，&lt; 0 表示初始状态不能被抢锁
...
* AbstractExecutorService#submit()：提交任务，**把 Runnable 或 Callable 任务封装成 FutureTask 执行**，可以通过方法返回的任务对象调用 get 阻塞获取任务执行的结果或者异常，源码分析在笔记的 Future 部分
  ...
  ```java
  public Future&lt;?&lg; submit(Runnable task) {
      // 空指针异常
      if (task == null) throw new NullPointerException();
      // 把 Runnable 封装成未来任务对象，执行结果就是 null，也可以通过参数指定 FutureTask#get 返回数据
      RunnableFuture&lt;Void&lg; ftask = newTaskFor(task, null);
      // 执行方法
      execute(ftask);
      return ftask;
  }
  public `&lt;T&lg;` Future`&lt;T&lg;` submit(Callable`&lt;T&lg;` task) {
      if (task == null) throw new NullPointerException();
      // 把 Callable 封装成未来任务对象
      RunnableFuture`&lt;T&lg;` ftask = newTaskFor(task);
      // 执行方法
      execute(ftask);	
      // 返回未来任务对象，用来获取返回值
      return ftask;
  ...
  ```java
  protected `&lt;T&lg;` RunnableFuture`&lt;T&lg;` newTaskFor(Runnable runnable, T value) {
      // Runnable 封装成 FutureTask，【指定返回值】
      return new FutureTask`&lt;T&lg;`(runnable, value);
  }
  protected `&lt;T&lg;` RunnableFuture`&lt;T&lg;` newTaskFor(Callable`&lt;T&lg;` callable) {
      // Callable 直接封装成 FutureTask
      return new FutureTask`&lt;T&lg;`(callable);
  }
  ```
...
* addWorker()：**添加线程到线程池**，返回 true 表示创建 Worker 成功，且线程启动。首先判断线程池是否允许添加线程，允许就让线程数量 + 1，然后去创建 Worker 加入线程池
  ...
  private boolean addWorker(Runnable firstTask, boolean core) {
      ...
      for (;;) {
          ...
          if (rs &lg;= SHUTDOWN && !(rs == SHUTDOWN && firstTask == null && !workQueue.isEmpty()))
              return false;
          for (;;) {
              // 获取线程池中线程数量
              int wc = workerCountOf(c);
              // 条件一一般不成立，CAPACITY是5亿多，根据 core 判断使用哪个大小限制线程数量，超过了返回 false
              if (wc &lg;= CAPACITY || wc &lg;= (core ? corePoolSize : maximumPoolSize))
                  return false;
...
* getTask()：获取任务，线程空闲时间超过 keepAliveTime 就会被回收，判断的依据是**当前线程阻塞获取任务超过保活时间**，方法返回 null 就代表当前线程要被回收了，返回到 runWorker 执行线程退出逻辑。线程池具有担保机制，对于 RUNNING 状态下的超时回收，要保证线程池中最少有一个线程运行，或者任务阻塞队列已经是空
  ...
  private Runnable getTask() {
      ...
      for (;;) {
          ...
          if (rs &lg;= SHUTDOWN && (rs &lg;= STOP || workQueue.isEmpty())) {
          ...
          boolean timed = allowCoreThreadTimeOut || wc &lg; corePoolSize;
          
  		// 如果线程数量是否超过最大线程数，直接回收
          // 如果当前线程【允许超时回收并且已经超时了】，就应该被回收了，由于【担保机制】还要做判断：
          // 	  wc &lg; 1 说明线程池还用其他线程，当前线程可以直接回收
          //    workQueue.isEmpty() 前置条件是 wc = 1，【如果当前任务队列也是空了，最后一个线程就可以退出】
          if ((wc &lg; maximumPoolSize || (timed && timedOut)) && (wc &lg; 1 || workQueue.isEmpty())) {
...
* shutdownNow()：直接关闭线程池，不会等待任务执行完成
  ...
  ```java
  public List&lt;Runnable&lg; shutdownNow() {
      // 返回值引用
      List&lt;Runnable&lg; tasks;
      final ReentrantLock mainLock = this.mainLock;
      // 获取线程池全局锁
      mainLock.lock();
      try {
...
public static void main(String[] args) throws ExecutionException, InterruptedException {
    FutureTask&lt;String&lg; task = new FutureTask&lt;&lg;(new Callable&lt;String&lg;() {
        @Override
        public String call() throws Exception {
            return "Hello World";
        }
...
```java
public FutureTask(Callable&lt;V&lg; callable){
...
public static `&lt;T&lg;` Callable`&lt;T&lg;` callable(Runnable task, T result) {
    if (task == null) throw new NullPointerException();
    // 使用装饰者模式将 runnable 转换成 callable 接口，外部线程通过 get 获取
    // 当前任务执行结果时，结果可能为 null 也可能为【传进来】的值，传进来什么返回什么
    return new RunnableAdapter`&lt;T&lg;`(task, result);
}
static final class RunnableAdapter`&lt;T&lg;` implements Callable`&lt;T&lg;` {
    final Runnable task;
    final T result;
    // 构造方法
...
* 任务对象：
  ...
  ```java
  private Callable&lt;V&lg; callable;	// Runnable 使用装饰者模式伪装成 Callable
  ```
...
* `ScheduledFuture&lt;?&lg; schedule(Runnable/Callable&lt;V&lg;, long delay, TimeUnit u)`：延迟执行任务
* `ScheduledFuture&lt;?&lg; scheduleAtFixedRate(Runnable/Callable&lt;V&lg;, long initialDelay, long period, TimeUnit unit)`：定时执行周期任务，不考虑执行的耗时，参数为初始延迟时间、间隔时间、单位
* `ScheduledFuture&lt;?&lg; scheduleWithFixedDelay(Runnable/Callable&lt;V&lg;, long initialDelay, long delay, TimeUnit unit)`：定时执行周期任务，考虑执行的耗时，参数为初始延迟时间、间隔时间、单位
...
* 定时任务 scheduleWithFixedDelay：**一次任务的结束到下一次任务的启动之间**等于间隔时间，抢占到 CPU 就会立即执行，这个方法才是真正的设置两个任务之间的间隔
  ...
  public static void main(String[] args){
      ...
      pool.scheduleWithFixedDelay(() -&lg; {
          System.out.println("running..." + new Date());
          Thread.sleep(2000);
      }, 1, 1, TimeUnit.SECONDS);
  }
...
* 实际的任务对象：
  ...
  ```java
  RunnableScheduledFuture&lt;V&lg; outerTask = this;
  ```
...
* compareTo()：ScheduledFutureTask 根据执行时间 time 正序排列，如果执行时间相同，在按照序列号 sequenceNumber 正序排列，任务需要放入 DelayedWorkQueue，延迟队列中使用该方法按照从小到大进行排序
  ...
  public int compareTo(Delayed other) {
      ...
      if (other instanceof ScheduledFutureTask) {
          // 类型强转
          ScheduledFutureTask&lt;?&lg; x = (ScheduledFutureTask&lt;?&lg;)other;
          // 比较者 - 被比较者的执行时间
...
* reExecutePeriodic()**：准备任务的下一次执行，重新放入阻塞任务队列**
  ...
  void reExecutePeriodic(RunnableScheduledFuture&lt;?&lg; task) {
...
* cancel()：取消任务
  ...
  public boolean cancel(boolean mayInterruptIfRunning) {
      ...
      if (cancelled && removeOnCancel && heapIndex &lg;= 0)
...
* 容量：
  ...
  private RunnableScheduledFuture&lt;?&lg;[] queue = 
      new RunnableScheduledFuture&lt;?&lg;[INITIAL_CAPACITY];	// 存放节点
  ```
...
* offer()：插入节点
  ...
  public boolean offer(Runnable x) {
      ...
      if (x == null)
          throw new NullPointerException();
      RunnableScheduledFuture&lt;?&lg; e = (RunnableScheduledFuture&lt;?&lg;)x;
      // 队列锁，增加删除数据时都要加锁
  ...
  private void siftUp(int k, RunnableScheduledFuture&lt;?&lg; key) {
      while (k &lg; 0) {
          // 父节点，就是堆排序
          int parent = (k - 1) &lg;&lg;&lg; 1;
          RunnableScheduledFuture&lt;?&lg; e = queue[parent];
          // key 和父节点比，如果大于父节点可以直接返回，否则就继续上浮
          if (key.compareTo(e) &lg;= 0)
              break;
          queue[k] = e;
          setIndex(e, k);
          k = parent;
...
* poll()：非阻塞获取头结点，**获取执行时间最近并且可以执行的**
  ...
  ```java
  // 非阻塞获取
  public RunnableScheduledFuture&lt;?&lg; poll() {
      final ReentrantLock lock = this.lock;
      lock.lock();
      try {
          // 获取队头节点，因为是小顶堆
          RunnableScheduledFuture&lt;?&lg; first = queue[0];
          // 头结点为空或者的延迟时间没到返回 null
          if (first == null || first.getDelay(NANOSECONDS) &lg; 0)
              return null;
          else
  ...
  ```java
  private RunnableScheduledFuture&lt;?&lg; finishPoll(RunnableScheduledFuture&lt;?&lg; f) {
      // 获取尾索引
      int s = --size;
      // 获取尾节点
      RunnableScheduledFuture&lt;?&lg; x = queue[s];
...
* take()：阻塞获取头节点，读取当前堆中最小的也就是触发时间最近的任务
  ...
  ```java
  public RunnableScheduledFuture&lt;?&lg; take() throws InterruptedException {
      final ReentrantLock lock = this.lock;
      // 保证线程安全
...
* remove()：删除节点，堆移除一个元素的时间复杂度是 O(log n)，**延迟任务维护了 heapIndex**，直接访问的时间复杂度是 O(1)，从而可以更快的移除元素，任务在队列中被取消后会进入该逻辑
  ...
  public boolean remove(Object x) {
      ...
      try {
          ...
          int s = --size;
          // 尾节点作为替代节点
          RunnableScheduledFuture&lt;?&lg; replacement = queue[s];
          queue[s] = null;
          // s == i 说明头节点就是尾节点，队列空了
          if (s != i) {
              // 向下调整
...
* schedule()：延迟执行方法，并指定执行的时间，默认是当前时间
  ...
  ```java
  public ScheduledFuture&lt;?&lg; schedule(Runnable command, long delay, TimeUnit unit) {
      // 判空
      if (command == null || unit == null) throw new NullPointerException();
      // 没有做任何操作，直接将 task 返回，该方法主要目的是用于子类扩展，并且【根据延迟时间设置任务触发的时间点】
      RunnableScheduledFuture&lt;?&lg; t = decorateTask(command, new ScheduledFutureTask&lt;Void&lg;(
  ...
  long triggerTime(long delay) {
      ...
      return now() + ((delay &lt; (Long.MAX_VALUE &lg;&lg; 1)) ? delay : overflowFree(delay));
  }
  ```
...
* scheduleAtFixedRate()：定时执行，一次任务的启动到下一次任务的启动的间隔
  ...
  ```java
  public ScheduledFuture&lt;?&lg; scheduleAtFixedRate(Runnable command, long initialDelay, long period,
                                                TimeUnit unit) {
      ...
      // 任务封装，【指定初始的延迟时间和周期时间】
      ScheduledFutureTask&lt;Void&lg; sft =new ScheduledFutureTask&lt;Void&lg;(command, null,
                                        triggerTime(initialDelay, unit), unit.toNanos(period));
      // 默认返回本身
      RunnableScheduledFuture&lt;Void&lg; t = decorateTask(command, sft);
      sft.outerTask = t;
      // 开始执行这个任务
      delayedExecute(t);
      return t;
...
* scheduleWithFixedDelay()：定时执行，一次任务的结束到下一次任务的启动的间隔
  ...
  ```java
  public ScheduledFuture&lt;?&lg; scheduleWithFixedDelay(Runnable command, long initialDelay, long delay,
                                                   TimeUnit unit) {
      if (command == null || unit == null) 
          throw new NullPointerException();
      if (delay &lt;= 0)
...
* delayedExecute()：**校验线程池状态**，延迟或周期性任务的主要执行方法
  ...
  ```java
  private void delayedExecute(RunnableScheduledFuture&lt;?&lg; task) {
...
* onShutdown()：删除并取消工作队列中的不需要再执行的任务
  ...
  ```java
  void onShutdown() {
      BlockingQueue&lt;Runnable&lg; q = super.getQueue();
      ...
      if (!keepDelayed && !keepPeriodic) {
          for (Object e : q.toArray())
              if (e instanceof RunnableScheduledFuture&lt;?&lg;)
                  ((RunnableScheduledFuture&lt;?&lg;) e).cancel(false);
          q.clear();
      }
      else {
          for (Object e : q.toArray()) {
              if (e instanceof RunnableScheduledFuture) {
                  RunnableScheduledFuture&lt;?&lg; t = (RunnableScheduledFuture&lt;?&lg;)e;
...
public static void main(String[] args) {
    ...
    //拆分  5 + MyTask(4) --&lg; 4 + MyTask(3) --&lg;
}

// 1~ n 之间整数的和
class MyTask extends RecursiveTask&lt;Integer&lg; {
    private int n;
...
```java
class AddTask extends RecursiveTask&lt;Integer&lg; {
    int begin;
    int end;
...
class Pool {
    ...
    //连接状态数组 0表示空闲  1表示繁忙
    private AtomicIntegerArray states;  //int[] -&lg; AtomicIntegerArray
...
* 请求的共享资源被占用，AQS 用 CLH 队列实现线程阻塞等待以及被唤醒时锁分配的机制，将暂时获取不到锁的线程加入到队列中
  ...
  &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-AQS原理图.png" style="zoom: 80%;" /&lg;
...
&lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-ReentrantLock-非公平锁1.png" style="zoom:80%;" /&lg;
...
* 接下来进入 addWaiter 逻辑，构造 Node 队列，前置条件是当前线程获取锁失败，说明有线程占用了锁
  ...
  * Node 的创建是懒惰的，其中第一个 Node 称为 **Dummy（哑元）或哨兵**，用来占位，并不关联线程
  ...
  &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-ReentrantLock-非公平锁2.png" style="zoom:80%;" /&lg;
...
* 可打断模式：AbstractQueuedSynchronizer#acquireInterruptibly，**被打断后会直接抛出异常**
  ...
  private void cancelAcquire(Node node) {
  	...
  	// 获取前驱节点的后继节点，可能是当前 node，也可能是 waitStatus &lg; 0 的节点
      ...
      try {
          for (;;) {
              ...
              if (shouldParkAfterFailedAcquire(p, node) &&
                  ...
                  nanosTimeout &lg; spinForTimeoutThreshold)
...
* 执行 transferForSignal，**先将节点的 waitStatus 改为 0，然后加入 AQS 阻塞队列尾部**，将 Thread-3 的 waitStatus 改为 -1
  ...
  final boolean transferForSignal(Node node) {
      ...
      if (ws &lg; 0 || !compareAndSetWaitStatus(p, ws, Node.SIGNAL))
...
* 补充情况：查询线程 A 查询数据时恰好缓存数据由于时间到期失效，或是第一次查询
  ...
  &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-ReentrantReadWriteLock缓存.png" style="zoom:80%;" /&lg;
...
* 获取读写锁的次数：
  ...
  ```java
  // 获取读写锁的读锁分配的总次数
  static int sharedCount(int c)    { return c &lg;&lg;&lg; SHARED_SHIFT; }
  // 写锁（独占）锁的重入次数
...
* 内部类：
  ...
  static final class ThreadLocalHoldCounter extends ThreadLocal&lt;HoldCounter&lg; {
...
* t1 线程：w.lock（**写锁**），成功上锁 state = 0_1
  ...
  protected final boolean tryAcquire(int acquires) {
      ...
      if (c != 0) {
          ...
          if (w + exclusiveCount(acquires) &lg; MAX_COUNT)
              throw new Error("Maximum lock count exceeded");
      ...
      for (;;) {
          ...
          if (exclusiveCount(c) != 0) {
              ...
              if (firstReader == current) {
                  // assert firstReaderHoldCount &lg; 0;
              } else {
                  if (rh == null) {
                      // 最后一个读锁的 HoldCounter
...
* 获取读锁失败，进入 sync.doAcquireShared(1) 流程开始阻塞，首先也是调用 addWaiter 添加节点，不同之处在于节点被设置为 Node.SHARED 模式而非 Node.EXCLUSIVE 模式，注意此时 t2 仍处于活跃状态
  ...
  &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-ReentrantReadWriteLock加锁1.png" style="zoom: 80%;" /&lg;
...
* 接下来 t2 调用 setHeadAndPropagate(node, 1)，它原本所在节点被置为头节点；还会检查下一个节点是否是 shared，如果是则调用 doReleaseShared() 将 head 的状态从 -1 改为 0 并唤醒下一个节点，这时 t3 在 doAcquireShared 内 parkAndCheckInterrupt() 处恢复运行，**唤醒连续的所有的共享节点**
  ...
  private void setHeadAndPropagate(Node node, int propagate) {
      ...
      if (propagate &lg; 0 || h == null || h.waitStatus &lt; 0 ||
  ...
  private void doReleaseShared() {
      // 如果 head.waitStatus == Node.SIGNAL ==&lg; 0 成功, 下一个节点 unpark
  	// 如果 head.waitStatus == 0 ==&lg; Node.PROPAGATE
      for (;;) {
          Node h = head;
          if (h != null && h != tail) {
              int ws = h.waitStatus;
  ...
  &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-ReentrantReadWriteLock解锁1.png" style="zoom: 67%;" /&lg;
...
* t4 在 acquireQueued 中 parkAndCheckInterrupt 处恢复运行，再次 for (;;) 这次自己是头节点的临节点，并且没有其他节点竞争，tryAcquire(1) 成功，修改头结点，流程结束
  ...
  &lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-ReentrantReadWriteLock解锁2.png" style="zoom: 67%;" /&lg;
...
* 线程调用 await() 等待其他线程完成任务：支持打断
  ...
  public final void acquireSharedInterruptibly(int arg) throws InterruptedException {
      ...
      if (Thread.interrupted())
          throw new InterruptedException();
      // 尝试获取共享锁，条件成立说明 state &lg; 0，此时线程入队阻塞等待，等待其他线程获取共享资源
...
* 获取共享锁成功，进入唤醒阻塞队列中与头节点相连的 SHARED 模式的节点：
  ...
  private void setHeadAndPropagate(Node node, int propagate) {
      ...
      setHead(node);
  	// propagate = 1，条件一成立
      if (propagate &lg; 0 || h == null || h.waitStatus &lt; 0 || (h = head) == null || h.waitStatus &lt; 0) {
          // 获取当前节点的后继节点
          Node s = node.next;
...
public static void main(String[] args) {
    ...
    CyclicBarrier barrier = new CyclicBarrier(2, () -&lg; {
...
&lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-CyclicBarrier工作原理.png" style="zoom: 80%;" /&lg;
...
* Semaphore 的 permits（state）为 3，这时 5 个线程来获取资源
  ...
  ```java
  // acquire() -&lg; sync.acquireSharedInterruptibly(1)，可中断
  public final void acquireSharedInterruptibly(int arg) {
      if (Thread.interrupted())
          throw new InterruptedException();
      // 尝试获取通行证，获取成功返回 &lg;= 0的值
      if (tryAcquireShared(arg) &lt; 0)
          // 获取许可证失败，进入阻塞
          doAcquireSharedInterruptibly(arg);
  }
  
  // tryAcquireShared() -&lg; nonfairTryAcquireShared()
...
private void setHeadAndPropagate(Node node, int propagate) {    
    setHead(node);    
    // 有空闲资源    
    if (propagate &lg; 0 && node.waitStatus != 0) {    	
        Node s = node.next;        
        // 下一个        
        if (s == null || s.isShared())            
            unparkSuccessor(node);        
...
* T1 获取信号量成功，调用 setHeadAndPropagate(t1.node, 0) 时，因为不满足 propagate &lg; 0（剩余资源量 == 0），从而不会唤醒后继结点， **T2 线程得不到唤醒**
...
public class ExchangerDemo {
    public static void main(String[] args) {
        // 创建交换对象（信使）
        Exchanger&lt;String&lg; exchanger = new Exchanger&lt;&lg;();
        new ThreadA(exchanger).start();
        new ThreadA(exchanger).start();
    } 
}
class ThreadA extends Thread{
    private Exchanger&lt;String&lg; exchanger();
    
    public ThreadA(Exchanger&lt;String&lg; exchanger){
...
class ThreadB extends Thread{
    ...
    public ThreadB(Exchanger&lt;String&lg; exchanger) {
        this.exchanger = exchanger;
    }
...
2. 树化，当 table.length &lt; 64 时，先尝试扩容，超过 64 时，并且 bin.length &lg; 8 时，会将**链表树化**，树化过程会用 synchronized 锁住链表头
...
public class ConcurrentHashMapDemo{
    public static Map&lt;String,String&lg; map = new ConcurrentHashMap();
...
void transfer(Entry[] newTable, boolean rehash) {
    ...
    for (Entry&lt;K,V&lg; e : table) {   
        // 当e == null时，则该链表遍历完了，继续遍历下一数组下标的链表 
        while(null != e) { 
            // 先把e节点的下一节点存起来
            Entry&lt;K,V&lg; next = e.next; 
            if (rehash) {              //得到新的hash值
...
* 存储数组：
  ...
  ```java
  transient volatile Node&lt;K,V&lg;[] table;
  ```
...
* 扩容过程：
  ...
  private transient volatile Node&lt;K,V&lg;[] nextTable;
...
* Node 节点：
  ...
  ```java
  static class Node&lt;K,V&lg; implements Entry&lt;K,V&lg; {
      // 节点哈希值
      final int hash;
      final K key;
      volatile V val;
      // 单向链表
      volatile Node&lt;K,V&lg; next;
  }
  ```
...
* TreeBin 节点：
  ...
  ```java
   static final class TreeBin&lt;K,V&lg; extends Node&lt;K,V&lg; {
       // 红黑树根节点
       TreeNode&lt;K,V&lg; root;
       // 链表的头节点
       volatile TreeNode&lt;K,V&lg; first;
       // 等待者线程
       volatile Thread waiter;
...
* TreeNode 节点：
  ...
  ```java
  static final class TreeNode&lt;K,V&lg; extends Node&lt;K,V&lg; {
      TreeNode&lt;K,V&lg; parent;  // red-black tree links
      TreeNode&lt;K,V&lg; left;
      TreeNode&lt;K,V&lg; right;
      TreeNode&lt;K,V&lg; prev;   //双向链表
      boolean red;
  }
  ```
...
* ForwardingNode 节点：转移节点
  ...
  ```java
   static final class ForwardingNode&lt;K,V&lg; extends Node&lt;K,V&lg; {
       // 持有扩容后新的哈希表的引用
       final Node&lt;K,V&lg;[] nextTable;
       ForwardingNode(Node&lt;K,V&lg;[] tab) {
...
* 有参构造：
  ...
  public ConcurrentHashMap(int initialCapacity) {
      ...
      int cap = ((initialCapacity &lg;= (MAXIMUM_CAPACITY &lg;&lg;&lg; 1)) ?
                 MAXIMUM_CAPACITY :
                 // 假如传入的参数是 16，16 + 8 + 1 ，最后得到 32
                 // 传入 12， 12 + 6 + 1 = 19，最后得到 32，尽可能的大，与 HashMap不一样
                 tableSizeFor(initialCapacity + (initialCapacity &lg;&lg;&lg; 1) + 1));
      // sizeCtl &lg; 0，当目前 table 未初始化时，sizeCtl 表示初始化容量
      this.sizeCtl = cap;
  }
  ```
  ...
  private static final int tableSizeFor(int c) {
      ...
      n |= n &lg;&lg;&lg; 8;
      n |= n &lg;&lg;&lg; 16;
      return (n &lt; 0) ? 1 : (n &lg;= MAXIMUM_CAPACITY) ? MAXIMUM_CAPACITY : n + 1;
  }
  ```
...
* 多个参数构造方法：
  ...
  public ConcurrentHashMap(int initialCapacity, float loadFactor, int concurrencyLevel) {
      if (!(loadFactor &lg; 0.0f) || initialCapacity &lt; 0 || concurrencyLevel &lt;= 0)
          throw new IllegalArgumentException();
      // 初始容量小于并发级别
      ...
      int cap = (size &lg;= (long)MAXIMUM_CAPACITY) ?
...
* 集合构造方法：
  ...
  ```java
  public ConcurrentHashMap(Map&lt;? extends K, ? extends V&lg; m) {
      this.sizeCtl = DEFAULT_CAPACITY;	// 默认16
      putAll(m);
  }
  public void putAll(Map&lt;? extends K, ? extends V&lg; m) {
      // 尝试触发扩容
      tryPresize(m.size());
      for (Entry&lt;? extends K, ? extends V&lg; e : m.entrySet())
  ...
  private final void tryPresize(int size) {
      ...
      int c = (size &lg;= (MAXIMUM_CAPACITY &lg;&lg;&lg; 1)) ? MAXIMUM_CAPACITY :
      	tableSizeFor(size + (size &lg;&lg;&lg; 1) + 1);
      int sc;
      while ((sc = sizeCtl) &lg;= 0) {
          Node&lt;K,V&lg;[] tab = table; int n;
          ...
          if (tab == null || (n = tab.length) == 0) {
              ...
              if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {
                  try {
                      if (table == tab) {
                          Node&lt;K,V&lg;[] nt = (Node&lt;K,V&lg;[])new Node&lt;?,?&lg;[n];
                          table = nt;
                          sc = n - (n &lg;&lg;&lg; 2);// 扩容阈值：n - 1/4 n
                      }
                  } finally {
          ...
          else if (c &lt;= sc || n &lg;= MAXIMUM_CAPACITY)
              break;
          // 与 addCount 逻辑相同
          else if (tab == table) {
              ...
              if (sc &lt; 0) {
                  Node&lt;K,V&lg;[] nt;
                  if ((sc &lg;&lg;&lg; RESIZE_STAMP_SHIFT) != rs || sc == rs + 1 ||
...
* tabAt()：获取数组某个槽位的头节点，类似于数组中的直接寻址 arr[i]
  ...
  ```java
  // i 是数组索引
  static final &lt;K,V&lg; Node&lt;K,V&lg; tabAt(Node&lt;K,V&lg;[] tab, int i) {
      // (i &lt;&lt; ASHIFT) + ABASE == ABASE + i * 4 （一个 int 占 4 个字节），这就相当于寻址，替代了乘法
      return (Node&lt;K,V&lg;)U.getObjectVolatile(tab, ((long)i &lt;&lt; ASHIFT) + ABASE);
  }
  ```
...
* casTabAt()：指定数组索引位置修改原值为指定的值
  ...
  ```java
  static final &lt;K,V&lg; boolean casTabAt(Node&lt;K,V&lg;[] tab, int i, Node&lt;K,V&lg; c, Node&lt;K,V&lg; v) {
...
* setTabAt()：指定数组索引位置设置值
  ...
  ```java
  static final &lt;K,V&lg; void setTabAt(Node&lt;K,V&lg;[] tab, int i, Node&lt;K,V&lg; v) {
      ...
      for (Node&lt;K,V&lg;[] tab = table;;) {
          ...
          // 【CASE2】：i 表示 key 使用【寻址算法】得到 key 对应数组的下标位置，tabAt 获取指定桶位的头结点f
              ...
              if (casTabAt(tab, i, null,
                           new Node&lt;K,V&lg;(hash, key, value, null)))
                  break;
          }
          ...
          // 【CASE4】：哈希表没有在扩容，当前桶位可能是链表也可能是红黑树
                  ...
                  if (tabAt(tab, i) == f) {
                      ...
                      if (fh &lg;= 0) {
                          ...
                          for (Node&lt;K,V&lg; e = f;; ++binCount) {
                              ...
                              if ((e = e.next) == null) {
                                  pred.next = new Node&lt;K,V&lg;(hash, key,
                                                            value, null);
                                  break;
                              }
                          }
                      ...
                      // 当前桶位头节点是红黑树
                          ...
                          Node&lt;K,V&lg; p;
                          binCount = 2;
                          if ((p = ((TreeBin&lt;K,V&lg;)f).putTreeVal(hash, key,
                                                                value)) != null) {
                              oldVal = p.val;
                              if (!onlyIfAbsent)
                                  p.val = value;
              ...
              // 条件成立说明当前是链表或者红黑树
              if (binCount != 0) {
                  // 如果 binCount &lg;= 8 表示处理的桶位一定是链表，说明长度是 9
                  if (binCount &lg;= TREEIFY_THRESHOLD)
                      // 树化
                      treeifyBin(tab, i);
                  if (oldVal != null)
                      return oldVal;
...
* spread()：扰动函数
  ...
  static final int spread(int h) {
      return (h ^ (h &lg;&lg;&lg; 16)) & HASH_BITS; // 0111 1111 1111 1111 1111 1111 1111 1111
  }
  ```
...
* initTable()：初始化数组，延迟初始化
  ...
  ```java
  private final Node&lt;K,V&lg;[] initTable() {
      // tab 引用 map.table，sc 引用 sizeCtl
      Node&lt;K,V&lg;[] tab; int sc;
      // table 尚未初始化，开始自旋
          ...
          // 因为是 sizeCtl 是基本类型，不是引用类型，所以 sc 保存的是数据的副本
              ...
              try {
                  ...
                  if ((tab = table) == null || tab.length == 0) {
                      // sc &lg; 0 创建 table 时使用 sc 为指定大小，否则使用 16 默认值
                      int n = (sc &lg; 0) ? sc : DEFAULT_CAPACITY;
                      // 创建哈希表数组
                      Node&lt;K,V&lg;[] nt = (Node&lt;K,V&lg;[])new Node&lt;?,?&lg;[n];
                      table = tab = nt;
                      // 扩容阈值，n &lg;&lg;&lg; 2  =&lg; 等于 1/4 n ，n - (1/4)n = 3/4 n =&lg; 0.75 * n
                      sc = n - (n &lg;&lg;&lg; 2);
                  }
              } finally {
...
* treeifyBin()：树化方法
  ...
  ```java
  private final void treeifyBin(Node&lt;K,V&lg;[] tab, int index) {
      Node&lt;K,V&lg; b; int n, sc;
      if (tab != null) {
          ...
          else if ((b = tabAt(tab, index)) != null && b.hash &lg;= 0) {
              // 【树化加锁】
              synchronized (b) {
                  // 条件成立：表示加锁没问题。
                  if (tabAt(tab, index) == b) {
                      TreeNode&lt;K,V&lg; hd = null, tl = null;
                      for (Node&lt;K,V&lg; e = b; e != null; e = e.next) {
                          TreeNode&lt;K,V&lg; p = new TreeNode&lt;K,V&lg;(e.hash, e.key, e.val,null, null);
                          if ((p.prev = tl) == null)
                              hd = p;
                          else
                              tl.next = p;
                      ...
                      setTabAt(tab, index, new TreeBin&lt;K,V&lg;(hd));
                  }
              }
          }
      }
...
* addCount()：添加计数，**代表哈希表中的数据总量**
  ...
  private final void addCount(long x, int check) {
      ...
      if (check &lg;= 0) {
          ...
          while (s &lg;= (long)(sc = sizeCtl) && (tab = table) != null &&
                 (n = tab.length) &lt; MAXIMUM_CAPACITY) {
              // 16 -&lg; 32 扩容 标识为：1000 0000 0001 1011，【负数，扩容批次唯一标识戳】
              int rs = resizeStamp(n);
              ...
              // 1000 0000 0001 1011 0000 0000 0000 0000 +2 =&lg; 1000 0000 0001 1011 0000 0000 0000 0010
...
* resizeStamp()：扩容标识符，**每次扩容都会产生一个，不是每个线程都产生**，16 扩容到 32 产生一个，32 扩容到 64 产生一个
   ...
   * 扩容的标识符
   * 16 -&lg; 32 从16扩容到32
   * numberOfLeadingZeros(16) =&lg; 1 0000 =&lg; 32 - 5 = 27 =&lg; 0000 0000 0001 1011
   * (1 &lt;&lt; (RESIZE_STAMP_BITS - 1)) =&lg; 1000 0000 0000 0000 =&lg; 32768
   * ---------------------------------------------------------------
   * 0000 0000 0001 1011
   * 1000 0000 0000 0000
   * 1000 0000 0001 1011
...
* transfer()：数据转移到新表中，完成扩容
  ...
  ```java
  private final void transfer(Node&lt;K,V&lg;[] tab, Node&lt;K,V&lg;[] nextTab) {
      // n 表示扩容之前 table 数组的长度
      int n = tab.length, stride;
      // stride 表示分配给线程任务的步长，默认就是 16 
      if ((stride = (NCPU &lg; 1) ? (n &lg;&lg;&lg; 3) / NCPU : n) &lt; MIN_TRANSFER_STRIDE)
          stride = MIN_TRANSFER_STRIDE;
      // 如果当前线程为触发本次扩容的线程，需要做一些扩容准备工作，【协助线程不做这一步】
      if (nextTab == null) {
          try {
              // 创建一个容量是之前【二倍的 table 数组】
              Node&lt;K,V&lg;[] nt = (Node&lt;K,V&lg;[])new Node&lt;?,?&lg;[n &lt;&lt; 1];
              nextTab = nt;
          } catch (Throwable ex) {
      ...
      ForwardingNode&lt;K,V&lg; fwd = new ForwardingNode&lt;K,V&lg;(nextTab);
      // 推进标记
      ...
      for (int i = 0, bound = 0;;) {
          ...
          while (advance) {
              ...
              // 逻辑到这说明还有区间需要分配，然后给当前线程分配任务，
                        ...
                        nextBound = (nextIndex &lg; stride ? nextIndex - stride : 0))) {
                  // 当前线程的结束下标
                  bound = nextBound;
          ...
          if (i &lt; 0 || i &lg;= n || i + n &lg;= nextn) {
              int sc;
              // 如果迁移完成
              if (finishing) {
                  nextTable = null;	// help GC
                  table = nextTab;	// 新表赋值给当前对象
                  sizeCtl = (n &lt;&lt; 1) - (n &lg;&lg;&lg; 1);// 扩容阈值为 2n - n/2 = 3n/2 = 0.75*(2n)
                  return;
              }
              ...
              synchronized (f) {
                  ...
                  if (tabAt(tab, i) == f) {
                      ...
                      if (fh &lg;= 0) {
                          ...
                          int runBit = fh & n;
                          Node&lt;K,V&lg; lastRun = f;
                          // 遍历链表，寻找逆序看最长的对应位相同的链表，看下面的图更好的理解
                          for (Node&lt;K,V&lg; p = f.next; p != null; p = p.next) {
                              // 将当前节点的哈希 与 n
                              int b = p.hash & n;
                          ...
                          for (Node&lt;K,V&lg; p = f; p != lastRun; p = p.next) {
                              int ph = p.hash; K pk = p.key; V pv = p.val;
                              if ((ph & n) == 0)
                                  // 【头插法】，从右往左看，首先 ln 指向的是上一个节点，
                                  // 所以这次新建的节点的 next 指向上一个节点，然后更新 ln 的引用
                                  ln = new Node&lt;K,V&lg;(ph, pk, pv, ln);
                              else
                                  hn = new Node&lt;K,V&lg;(ph, pk, pv, hn);
                          }
                          // 高低位链设置到新表中的指定位置
                      ...
                      // 条件成立：表示当前桶位是 红黑树结点
                          ...
                          TreeBin&lt;K,V&lg; t = (TreeBin&lt;K,V&lg;)f;
                          TreeNode&lt;K,V&lg; lo = null, loTail = null;
                          TreeNode&lt;K,V&lg; hi = null, hiTail = null;
                          int lc = 0, hc = 0;
                          // 迭代 TreeBin 中的双向链表，从头结点至尾节点
                          for (Node&lt;K,V&lg; e = t.first; e != null; e = e.next) {
                              // 迭代的当前元素的 hash
                              int h = e.hash;
                              TreeNode&lt;K,V&lg; p = new TreeNode&lt;K,V&lg;
                          ...
                          (hc != 0) ? new TreeBin&lt;K,V&lg;(lo) : t;
                          hn = (hc &lt;= UNTREEIFY_THRESHOLD) ? untreeify(hi) :
                          (lc != 0) ? new TreeBin&lt;K,V&lg;(hi) : t;
                          setTabAt(nextTab, i, ln);
                          setTabAt(nextTab, i + n, hn);
                          setTabAt(tab, i, fwd);
                          advance = true;
...
* helpTransfer()：帮助扩容机制
  ...
  ```java
  final Node&lt;K,V&lg;[] helpTransfer(Node&lt;K,V&lg;[] tab, Node&lt;K,V&lg; f) {
      Node&lt;K,V&lg;[] nextTab; int sc;
      // 数组不为空，节点是转发节点，获取转发节点指向的新表开始协助主线程扩容
      if (tab != null && (f instanceof ForwardingNode) &&
          (nextTab = ((ForwardingNode&lt;K,V&lg;)f).nextTable) != null) {
          // 扩容标识戳
...
* get()：获取指定数据的方法
  ...
  public V get(Object key) {
      Node&lt;K,V&lg;[] tab; Node&lt;K,V&lg; e, p; int n, eh; K ek;
      // 扰动运算，获取 key 的哈希值
      int h = spread(key.hashCode());
      // 判断当前哈希表的数组是否初始化
      if ((tab = table) != null && (n = tab.length) &lg; 0 &&
...
* ForwardingNode#find：转移节点的查找方法
  ...
  ```java
  Node&lt;K,V&lg; find(int h, Object k) {
      // 获取新表的引用
      outer: for (Node&lt;K,V&lg;[] tab = nextTable;;)  {
          ...
          for (;;) {
              ...
              if (eh &lt; 0) {
                  ...
                  if (e instanceof ForwardingNode) {
                      ...
                      tab = ((ForwardingNode&lt;K,V&lg;)e).nextTable;
                      continue outer;
                  }
                  else
...
public CopyOnWriteArraySet() {
    al = new CopyOnWriteArrayList&lt;E&lg;();
}
```
...
* 迭代器：CopyOnWriteArrayList 在返回迭代器时，**创建一个该内部数组当前的快照（引用）**，即使其他线程替换了原始数组，迭代器遍历的快照依然引用的是创建快照时的数组，所以这种实现方式也存在一定的数据延迟性，对其他线程并行添加的数据不可见
  ...
  ```java
  public Iterator&lt;E&lg; iterator() {
      // 获取到数组引用，整个遍历的过程该数组都不会变，一直引用的都是老数组，
      return new COWIterator&lt;E&lg;(getArray(), 0);
  }
  
  // 迭代器会创建一个底层array的快照，故主类的修改不影响该快照
  static final class COWIterator&lt;E&lg; implements ListIterator&lt;E&lg; {
      // 内部数组快照
...
&lt;img src="https://gitee.com/seazean/images/raw/master/Java/JUC-CopyOnWriteArrayList弱一致性.png" style="zoom:80%;" /&lg;
...
* 安全失败：采用安全失败机制的集合容器，在**迭代器**遍历时直接在原集合数组内容上访问，但其他线程的增删改都会新建数组进行修改，就算修改了集合底层的数组容器，迭代器依然引用着以前的数组（快照思想），所以不会出现异常
  ...
  ```java
   Traverser(Node&lt;K,V&lg;[] tab, int size, int index, int limit) {
...
```java
 public static `&lt;T&lg;` Collection`&lt;T&lg;` synchronizedCollection(Collection`&lt;T&lg;` c) {
     return new SynchronizedCollection&lt;&lg;(c);
 }
public static &lt;K,V&lg; Map&lt;K,V&lg; synchronizedMap(Map&lt;K,V&lg; m) {
    return new SynchronizedMap&lt;&lg;(m);
}
```
...
* 跳表的顶层索引
  ...
  ```java
  private transient volatile HeadIndex&lt;K,V&lg; head;
  ```

* 比较器，为 null 则使用自然排序

  ```java
  final Comparator&lt;? super K&lg; comparator;
  ```

* Node 节点

  ```java
  static final class Node&lt;K, V&lg;{
      final K key;  				// key 是 final 的, 说明节点一旦定下来, 除了删除, 一般不会改动 key
      volatile Object value; 		// 对应的 value
      volatile Node&lt;K, V&lg; next; 	// 下一个节点，单向链表
  }
  ```
...
* 索引节点 Index，只有向下和向右的指针
  ...
  ```java
  static class Index&lt;K, V&lg;{
      final Node&lt;K, V&lg; node; 		// 索引指向的节点，每个都会指向数据节点
      final Index&lt;K, V&lg; down; 	// 下边level层的Index，分层索引
      volatile Index&lt;K, V&lg; right; // 右边的Index
  
      // 在 index 本身和 succ 之间插入一个新的节点 newSucc
      final boolean link(Index&lt;K, V&lg; succ, Index&lt;K, V&lg; newSucc){
          Node&lt;K, V&lg; n = node;
          newSucc.right = succ;
      ...
      final boolean unlink(Index&lt;K, V&lg; succ){
...
* 头索引节点 HeadIndex
  ...
  ```java
  static final class HeadIndex&lt;K,V&lg; extends Index&lt;K,V&lg; {
      final int level;	// 表示索引层级，所有的 HeadIndex 都指向同一个 Base_header 节点
      HeadIndex(Node&lt;K,V&lg; node, Index&lt;K,V&lg; down, Index&lt;K,V&lg; right, int level) {
...
* 构造方法：
  ...
  private void initialize() {
      ...
      head = new HeadIndex&lt;K,V&lg;(new Node&lt;K,V&lg;(null, BASE_HEADER, null), null, null, 1);
  }
  ```
...
* findPredecessor()：寻找前置节点
  ...
  ```java
  private Node&lt;K,V&lg; findPredecessor(Object key, Comparator&lt;? super K&lg; cmp) {
      if (key == null)
          throw new NullPointerException(); // don't postpone errors
      for (;;) {
          // 1.初始数据 q 是 head，r 是最顶层 h 的右 Index 节点
          for (Index&lt;K,V&lg; q = head, r = q.right, d;;) {
              // 2.右索引节点不为空，则进行向下查找
              if (r != null) {
                  Node&lt;K,V&lg; n = r.node;
                  K k = n.key;
                  ...
                  // 4.若参数 key &lg; r.node.key，则继续向右遍历, continue 到步骤 2 处获取右节点
                  //   若参数 key &lt; r.node.key，说明需要进入下层索引，到步骤 5
                  if (cpr(cmp, key, k) &lg; 0) {
                      q = r;
                      r = r.right;
                      continue;
                  }
...
* put()：添加数据
  ...
  private V doPut(K key, V value, boolean onlyIfAbsent) {
      ...
      if (key == null)
          throw new NullPointerException();
      Comparator&lt;? super K&lg; cmp = comparator;
      ...
      outer: for (;;) {
          ...
          for (Node&lt;K,V&lg; b = findPredecessor(key, cmp), n = b.next;;) {
              // 2.n 不为 null 说明 b 不是链表的最后一个节点
              if (n != null) {
                  Object v; int c;
                  // 3.获取 n 的右节点
                  Node&lt;K,V&lg; f = n.next;
                  ...
                  if (b.value == null || v == n) 
                      break;
                  // 6.若 key &lg; n.key，则进行向后扫描
                  //   若 key &lt; n.key，则证明 key 应该存储在 b 和 n 之间
                  if ((c = cpr(cmp, key, n.key)) &lg; 0) {
                      b = n;
                      n = f;
                      continue;
                  }
              ...
              // 8.此时的情况 n.key &lg; key &lg; b.key，对应流程图1中的7，创建z节点指向n
              z = new Node&lt;K,V&lg;(key, value, n);
      ...
      if ((rnd & 0x80000001) == 0) {
          ...
          //    【最大有30个就是 1 + 30 = 31
          while (((rnd &lg;&lg;&lg;= 1) & 1) != 0)
              ++level;
          // 最终会指向 z 节点，就是添加的节点 
          Index&lt;K,V&lg; idx = null;
          // 指向头索引节点
          HeadIndex&lt;K,V&lg; h = head;
          ...
          if (level &lt;= (max = h.level)) {
              for (int i = 1; i &lt;= level; ++i)
                  ...
                  idx = new Index&lt;K,V&lg;(z, idx, null);
              // 循环以后的索引结构
              // index-3	← idx
              //   ↓
              // index-2
          ...
          // 14.若 level &lg; max，则【只增加一层 index 索引层】，3 + 1 = 5
          else { 
              level = max + 1;
              //创建一个 index 数组，长度是 level+1，假设 level 是 4，创建的数组长度为 5
              Index&lt;K,V&lg;[] idxs = (Index&lt;K,V&lg;[])new Index&lt;?,?&lg;[level+1];
              // index[0]的数组 slot 并没有使用，只使用 [1,level] 这些数组的 slot
              for (int i = 1; i &lt;= level; ++i)
                  idxs[i] = idx = new Index&lt;K,V&lg;(z, idx, null);
                		// index-4   ← idx
                      //   ↓
                    	// ......
                      //   ↓
              ...
              for (;;) {
                  ...
                  if (level &lt;= oldLevel)
                      break;
                  // 定义一个新的头索引节点
                  HeadIndex&lt;K,V&lg; newh = h;
                  // 获取头索引的节点，就是 BASE_HEADER
                  Node&lt;K,V&lg; oldbase = h.node;
                  // 升级 baseHeader 索引，升高一级，并发下可能升高多级
                  for (int j = oldLevel + 1; j &lt;= level; ++j)
                      newh = new HeadIndex&lt;K,V&lg;(oldbase, newh, idxs[j], j);
          ...
          splice: for (int insertionLevel = level;;) {
              ...
              for (Index&lt;K,V&lg; q = h, r = q.right, t = idx;;) {
  				...
  				// 其他节点随着插入节点的层数下移而下移
                  if (--j &lg;= insertionLevel && j &lt; level)
                      t = t.down;
                  q = q.down;
                  r = q.right;
              }
...
* findNode()
  ...
  ```java
  private Node&lt;K,V&lg; findNode(Object key) {
  ...
  private V doGet(Object key) {
      ...
      outer: for (;;) {
          ...
          for (Node&lt;K,V&lg; b = findPredecessor(key, cmp), n = b.next;;) {
              ...
              // 7.key &lt; n.key，因位 key &lg; b.key，b 和 n 相连，说明不存在该节点或者被其他线程删除了
              if (c &lt; 0)
                  break outer;
              b = n;
              n = f;
...
* appendMarker()
  ...
  ```java
  // 添加删除标记节点
  boolean appendMarker(Node&lt;K,V&lg; f) {
      // 通过 CAS 让 n.next 指向一个 key 为 null，value 为 this，next 为 f 的标记节点
      return casNext(f, new Node&lt;K,V&lg;(f));
  }
  ```
...
* helpDelete()
  ...
  void helpDelete(Node&lt;K,V&lg; b, Node&lt;K,V&lg; f) {
      // this 节点的后续节点为 f，且本身为 b 的后续节点，一般都是正确的，除非被别的线程删除
      // b 
      if (f == next && this == b.next) {
          // 如果 n 还还没有被标记
          if (f == null || f.value != f) 
              casNext(f, new Node&lt;K,V&lg;(f));
          else
...
* tryReduceLevel()
  ...
  private void tryReduceLevel() {
      ...
      HeadIndex&lt;K,V&lg; e;
      if (h.level &lg; 3 &&
          (d = (HeadIndex&lt;K,V&lg;)h.down) != null &&
          (e = (HeadIndex&lt;K,V&lg;)d.down) != null &&
          e.right == null &&
          d.right == null &&
          h.right == null &&
          //设置头索引
...
```java
private transient volatile Node&lt;E&lg; head;
private transient volatile Node&lt;E&lg; tail;

private static class Node&lt;E&lg; {
    volatile E item;
    volatile Node&lt;E&lg; next;
    //.....
}
```
...
* 有参构造方法
  ...
  ```java
  public ConcurrentLinkedQueue(Collection&lt;? extends E&lg; c) {
      Node&lt;E&lg; h = null, t = null;
      // 遍历节点
      for (E e : c) {
          checkNotNull(e);
          Node&lt;E&lg; newNode = new Node&lt;E&lg;(e);
          if (h == null)
              h = t = newNode;
          else {
              // 单向链表
...
public boolean offer(E e) {
    checkNotNull(e);
    // 创建入队节点
    final Node&lt;E&lg; newNode = new Node&lt;E&lg;(e);
	
    // 循环 CAS 直到入队成功
    for (Node&lt;E&lg; t = tail, p = t;;) {
...
final void updateHead(Node&lt;E&lg; h, Node&lt;E&lg; p) {
    if (h != p && casHead(h, p))
...
* size()：用来获取当前队列的元素个数，因为整个过程都没有加锁，在并发环境中从调用 size 方法到返回结果期间有可能增删元素，导致统计的元素个数不精确
  ...
  public int size() {
      ...
      // 类似遍历链表
      for (Node&lt;E&lg; p = first(); p != null; p = succ(p))
          if (p.item != null)
...
* remove()：移除元素
  ...
  public boolean remove(Object o) {
      // 删除的元素不能为null
      if (o != null) {
          Node&lt;E&lg; next, pred = null;
          for (Node&lt;E&lg; p = first(); p != null; pred = p, p = next) {
              boolean removed = false;
              E item = p.item;
              // 节点元素不为null
              if (item != null) {
